...
<head>
 ...
 @media (max-width: 480px) { 
  ...
  <script>
    const API_BASE = "./api/portal_api.php";
...
<body>
  ...
  <div class="event-marquee-container" id="marqueeContainer" style="display: none;">
    <div class="marquee-label">Upcoming</div>
    <div class="event-marquee" id="eventMarquee"></div>
  </div>
  ...
  <script>
    ...
    function getStatusTag(status) {
      switch(status) {
        case 'confirmed':
          return `<div class="tag" style="background: rgba(0,212,180,0.1); color: var(--teal); border: 1px solid rgba(0,212,180,0.3); padding: 4px 12px; font-size: 0.7rem; border-radius: 6px;">✓ Registered</div>`;
        case 'awaiting_confirmation':
          return `<div class="tag" style="background: rgba(245,166,35,0.1); color: var(--accent); border: 1px solid rgba(245,166,35,0.3); padding: 4px 12px; font-size: 0.7rem; border-radius: 6px;">⌛ Pending Approval</div>`;
        case 'pending_payment':
          return `<div class="tag" style="background: rgba(232,55,107,0.1); color: var(--accent2); border: 1px solid rgba(232,55,107,0.3); padding: 4px 12px; font-size: 0.7rem; border-radius: 6px;">💳 Payment Pending</div>`;
        case 'waitlisted':
          return `<div class="tag" style="background: rgba(132,144,200,0.1); color: var(--muted); border: 1px solid rgba(132,144,200,0.3); padding: 4px 12px; font-size: 0.7rem; border-radius: 6px;">📋 Waitlisted</div>`;
        case 'rejected':
          return `<div class="tag" style="background: rgba(239,68,68,0.1); color: #ef4444; border: 1px solid rgba(239,68,68,0.3); padding: 4px 12px; font-size: 0.7rem; border-radius: 6px;">✕ Rejected</div>`;
        default:
          return `<div class="tag" style="background: rgba(0,212,180,0.1); color: var(--teal); border: 1px solid rgba(0,212,180,0.3); padding: 4px 12px; font-size: 0.7rem; border-radius: 6px;">✓ Registered</div>`;
      }
    }
    ...
    function renderEvents() {
      ...
      if (events && events.length > 0) {
        ...
        const marqueeContent = events.map(e => `<span><span class="m-dot">⚡</span> ${e.title} · ${new Date(e.datetime).toLocaleDateString('en-IN', {month:'short', day:'numeric'})}</span>`).join("");
        marquee.innerHTML = marqueeContent + marqueeContent + marqueeContent; // Multiple duplicates for long scroll
      } else {
      ...
      grid.innerHTML = filtered.map(e => `
        <div class="event-card" data-id="${e.id}">
          ...
          <div class="card-footer">
            ...
            ${e.isRegistered ? 
              getStatusTag(e.registrationStatus) :
              (currentUser ? 
    ...
    function getModalStatusButton(status) {
      switch(status) {
        case 'confirmed':
          return `<div class="btn" style="width: 100%; justify-content: center; padding: 12px; font-weight: 600; background: rgba(0,212,180,0.1); color: var(--teal); border: 1px solid rgba(0,212,180,0.3); cursor: default;">✓ Registered for this Event</div>`;
        case 'awaiting_confirmation':
          return `<div class="btn" style="width: 100%; justify-content: center; padding: 12px; font-weight: 600; background: rgba(245,166,35,0.1); color: var(--accent); border: 1px solid rgba(245,166,35,0.3); cursor: default;">⌛ Pending Admin Approval</div>`;
        case 'pending_payment':
          return `<div class="btn" style="width: 100%; justify-content: center; padding: 12px; font-weight: 600; background: rgba(232,55,107,0.1); color: var(--accent2); border: 1px solid rgba(232,55,107,0.3); cursor: default;">💳 Pending Payment Proof</div>`;
        case 'waitlisted':
          return `<div class="btn" style="width: 100%; justify-content: center; padding: 12px; font-weight: 600; background: rgba(132,144,200,0.1); color: var(--muted); border: 1px solid rgba(132,144,200,0.3); cursor: default;">📋 On Waitlist</div>`;
        case 'rejected':
          return `<div class="btn" style="width: 100%; justify-content: center; padding: 12px; font-weight: 600; background: rgba(239,68,68,0.1); color: #ef4444; border: 1px solid rgba(239,68,68,0.3); cursor: default;">✕ Registration Rejected</div>`;
        default:
          return `<div class="btn" style="width: 100%; justify-content: center; padding: 12px; font-weight: 600; background: rgba(0,212,180,0.1); color: var(--teal); border: 1px solid rgba(0,212,180,0.3); cursor: default;">✓ Registered for this Event</div>`;
      }
    }
    ...
    function openEventModal(id) {
      ...
      document.getElementById("eventModalContent").innerHTML = `
        ...
        <div style="margin-top: 2rem; display: flex; flex-direction: column; gap: 10px;">
          ${e.isRegistered ? 
            getModalStatusButton(e.registrationStatus) :
            (currentUser ? 
